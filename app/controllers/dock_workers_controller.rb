class DockWorkersController < ApplicationController
  before_action :set_dock_worker,
                only: [:show, :edit, :update, :archived, :destroy, :payment, :make_payment]

  # GET /dock_workers
  # GET /dock_workers.json
  def index
    @dock_workers = DockWorker.where.not(archived: true).paginate(
  ...
  def create
    ...
    respond_to do |format|
      if @dock_worker.save
        format.html do
          redirect_to @dock_worker,
                      notice: 'Dock worker was successfully created.'
        end
        format.json { render :show, status: :created, location: @dock_worker }
      else
        format.html { render :new }
        format.json do
          render json: @dock_worker.errors,
  ...
  def update
    respond_to do |format|
      if @dock_worker.update(dock_worker_params)
        format.html do
          redirect_to @dock_worker,
                      notice: 'Dock worker was successfully updated.'
        end
  ...
  def archived
    ...
    redirect_to dock_workers_path,
                notice: 'Dock worker was successfully archived.'
  end
  ...
  def destroy
    ...
    respond_to do |format|
      format.html do
        redirect_to dock_workers_url,
                    notice: 'Dock worker was successfully destroyed.'
      end
  ...
  def payment
    @timeslots = @dock_worker.timeslots.where(
      date: (@dock_worker.last_payment_date..Date.yesterday)
    )
  ...
  def avg_payroll
    ...
    @timeslots.each do |timeslot|
      @total_appointments += timeslot.appointments.count
      timeslot.dock_workers.each do |dock_worker|
        @total_payroll += dock_worker.pay_rate
      end
    end
    @avg_payroll = if @total_appointments != 0
                     @total_payroll / @total_appointments
                   else
                     0
                   end
    respond_to do |format|
  ...
  def dock_worker_params
    params.require(:dock_worker).permit(
      :last_name, :first_name, :address, :state_id,
      :zipcode, :pay_rate, :archived
    )
  end
end
